...
def open_app(name: str) -> dict[str, Any]:
    """
    Open any app by name and return structured action data.

    Text responses are intentionally generated by callers so this function can
    stay focused on the action result payload.
    """
    ...
    if resolved:
        ...
        app_name = str(resolved.get("name") or canonical)
        entity_id = location or app_name
    ...
    if platform.system() == "Windows":
        try:
            ...
            return _action_payload(
                ...
                entity_id=canonical,
                entity_label=canonical,
                error=f"start command failed: {e}",
            )
